...
export default function UserReviewsSection({ user }) {
      ...
      <div>
        ...
        {userProducts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {userProducts.map(product => (
              <div key={product.id} className="bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 
                                              rounded-xl p-4 md:p-6 hover:shadow-xl transition-all duration-300 
                                              hover:border-purple-300 group">
                <div className="font-bold text-base md:text-lg text-gray-900 mb-3 group-hover:text-purple-600 transition-colors">
                  {product.name}
                </div>
          ...
          <div className="text-center py-12 md:py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl">
            <Package className="w-12 md:w-16 h-12 md:h-16 mx-auto mb-4 text-gray-300" />
            <p className="text-gray-500 text-base md:text-lg">No products available</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div>
      <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-6 md:mb-8 flex items-center gap-2 md:gap-3">
        <div className="p-1.5 md:p-2 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-lg">
          <MessageSquare className="w-5 md:w-6 h-5 md:h-6 text-white" />
        </div>
        Customer Reviews
      </h3>
      <div className="mb-4 md:mb-6">
        <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-4 md:p-6 rounded-xl border border-yellow-200/50">
          <p className="text-xs md:text-sm text-gray-600 mb-1">Total Reviews</p>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">{userReviews.length}</p>
        </div>
      </div>
      {userReviews.length > 0 ? (
        <div className="space-y-4">
          {userReviews.map(review => (
            <div key={review.id} className="bg-white border-2 border-gray-200 rounded-xl p-4 md:p-6 
                                          hover:shadow-xl transition-all duration-300 hover:border-yellow-300">
              <div className="flex items-center justify-between mb-3">
                <div className="font-bold text-base md:text-lg text-gray-900">{review.product}</div>
              ...
              <p className="text-sm md:text-base text-gray-600 mb-3">{review.comment}</p>
        ...
        <div className="text-center py-12 md:py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl">
          ...
          <p className="text-gray-500 text-base md:text-lg">No reviews available</p>
        </div>
      )}
    </div>
  );
